...
class Config:
    ...
    model_path = 'mask_rcnn/mask_rcnn_model_node.pth'
    test_image_dir = 'dataset_coco_neuro_2/val/images'
    test_annotations_dir = 'dataset_coco_neuro_2/val/annotations'
    masks_folder = 'dataset_coco_neuro_2/masks'
    output_dir = 'predict_mask_rcnn_node_metrics'
...
class_metrics = {
    'Node': {'true_pos': 0, 'false_pos': 0, 'false_neg': 0}
}
...
def plot_ground_truth(ax, image, ground_truth, title, true_mask=None):
    ...
    if ground_truth is not None:
        for ann in ground_truth:
            ...
            ax.text(box[0], box[1] - 10, class_name, color='green',
                    fontsize=12, backgroundcolor='white')

    if true_mask is not None:
        image_with_mask = np.array(image).astype(np.float32) / 255.0
        class_mask = (true_mask > 0)
        color = (0, 1, 0)
...
def convert_to_coco_format(predictions, image_id, height, width):
    ...
    keep = (scores >= Config.confidence_threshold) & (labels != 0)
    boxes = boxes[keep]
    labels = labels[keep]
    scores = scores[keep]
    return {
...
def calculate_map(all_preds, all_targets):
    ...
    map50 = results_50['map'].item() if 'map' in results_50 else 0.0
    map50_95 = results_50_95['map'].item() if 'map' in results_50_95 else 0.0

    class_wise_map50 = []
    if 'map_per_class' in results_50 and results_50['map_per_class'] is not None:
        if results_50['map_per_class'].dim() == 0:
            class_wise_map50 = [results_50['map_per_class'].item()]
        else:
            class_wise_map50 = [x.item() for x in results_50['map_per_class']]

    class_wise_map50_95 = []
    if 'map_per_class' in results_50_95 and results_50_95['map_per_class'] is not None:
        if results_50_95['map_per_class'].dim() == 0:
            class_wise_map50_95 = [results_50_95['map_per_class'].item()]
        else:
            class_wise_map50_95 = [x.item() for x in results_50_95['map_per_class']]
